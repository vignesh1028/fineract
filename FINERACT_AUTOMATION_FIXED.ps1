...
function Phase2-SystemConfiguration {
    ...
    # Use default office (ID=1) - no need to create
    $officeId = 1
    Write-Host "`n[2.1] Using Default Office..." -ForegroundColor Cyan
    Write-Host "Office ID: $officeId" -ForegroundColor Green
...
function Phase3-LoanProductConfiguration {
    ...
    if ($null -eq $product) {
        Write-Host "[3.1] Failed to create loan product" -ForegroundColor Red
        return $null
    }
...
function Phase5-LoanApplication {
    ...
    if ($null -eq $loan) {
        Write-Host "[5.1] Failed to create loan" -ForegroundColor Red
        return $null
    }
    ...
    $loanDetails = Invoke-FineractAPI -Method GET -Endpoint "/loans/$loanId"
...
function Phase7-Reports {
    ...
    Write-Host "`n========== PHASE 7: REPORTS ==========" -ForegroundColor Yellow
    ...
    $schedule = Invoke-FineractAPI -Method GET -Endpoint "/loans/$LoanId"
...
# MAIN EXECUTION
Write-Host "`nApache Fineract Microloan Assignment Automation" -ForegroundColor Magenta
...
Write-Host "`nAssignment Completed!" -ForegroundColor Green
